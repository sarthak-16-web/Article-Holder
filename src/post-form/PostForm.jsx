...
//          // yaha tak na userdata aa rha na uski id ;
//         if (post) {
//             const file = data.image[0] ? await services.UploadFile(data.image[0]) : null;

//             if (file) {
//                 services.DeletePost(post.featured_Image);
//             }
...
//                         <img
//                             src={services.getFilePreview(post.featured_Image)}
//                             alt={post.title}
...
import React, { useCallback } from "react"
import { useForm } from "react-hook-form"
import { Input, RTE, Select, Button } from "../components/import_component" // Assuming these components accept className
import services from "../appwrite/database_services"
import { useNavigate } from "react-router-dom"
import { useSelector } from "react-redux"
...
export default function PostForm({ post }) {
  ...
  const submit = async (data) => {
    ...
    if (post) {
      const file = data.image[0] ? await services.UploadFile(data.image[0]) : null
      if (file) {
        services.DeleteFile(post.featured_Image) // Corrected from DeletePost to DeleteFile
      }
      const dbPost = await services.updatePost(post.$id, {
        ...data,
        featured_Image: file ? file.$id : undefined,
